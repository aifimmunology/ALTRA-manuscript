...
##### regenerate leiden options grid
for label, sub_adata in subset_data.items():
    print("Key:", label)
    print("Value:", sub_adata)
    
    p1 = sy.plot.umap(sub_adata, color= ['leiden_res_2', 'leiden_res_2.5', 'leiden_res_3', 'leiden_res_1.0'] ,return_fig = True, size = .5, ncols = 2,wspace=1)
    p1.savefig(leiden_fig_path + label   + "_umap_leiden_resolutions_" + panel + ".png",  dpi=400)